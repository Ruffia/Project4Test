...
int main(int argc, char const *argv[])
{
	...
	//root_node.append_attribute("PageNumber").set_value("书本数量");
	//pugi::xml_node name_node = root_node.append_child("name");
	//name_node.append_child(pugi::node_pcdata).set_value("小王子");
	//xml_doc.save_file("pugixml_Example_MultiByte.xml");
	string fileName = "MarcoEnvironment.xml";
	xml_document doc;
	if (!doc.load_file(fileName.c_str())) { //加载xml文件
		return false;
	}
	...
	xml_node nodeWidget = nodeLayout.child("widget");
	xml_node node = nodeLayout.first_child();
	while (!node.empty())
	{
		string s = node.attribute("Caption").as_string();
