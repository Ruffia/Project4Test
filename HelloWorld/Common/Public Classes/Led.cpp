...
#include "stdafx.h"
#include "..\..\Language\LanguagePackage\resource.h"
#include "Led.h"
...
CLed::CLed()
{
	m_LedBitmap.LoadBitmap(IDB_BITMAP_LEDS);
...
void CLed::ResetLed(int nLedColor, int nMode, int nShape)
{
...
void CLed::SetLedMode(int nMode)
{
	ResetLed(m_nLedColor,nMode,m_nLedShape);
}
...
void CLed::Ping(DWORD dwTimeout)
{
	...
	m_bPingEnabled = TRUE;
	ResetLed(m_nLedColor,CLed::LED_ON,m_nLedShape);
...
void CLed::OnTimer(UINT nIDEvent) 
{
	...
	{
		ResetLed(m_nLedColor,CLed::LED_OFF,m_nLedShape);
		KillTimer(nIDEvent);
