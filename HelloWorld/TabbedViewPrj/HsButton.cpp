...
int HsButton::m_nRefs = 0;                               // # HsButton instances
CMapStringToPtr HsButton::m_btnGroups;                   // button groups
HINSTANCE HsButton::m_hMsimg32 = NULL;                   // handle to MSIMG32.dll
MSIMG32callBack HsButton::m_dllGradientFillFunc = NULL;  // pointer to GradientFill() API

// Gradient colors
COLORREF HsButton::m_rgbTopGradient = RGB (255, 255, 255);
COLORREF HsButton::m_rgbBottomGradient = RGB (200, 200, 216);
COLORREF HsButton::m_rgbTopGradientHot = RGB (255, 255, 255);
COLORREF HsButton::m_rgbBottomGradientHot = RGB (220, 220, 220);
...
HsButton::HsButton()
{
  ...
  setType (HsButton::pushButton);
  setTextStyle (HsButton::singleLineCenter);
  setTextColor (GetSysColor (COLOR_BTNTEXT));
  setFocusStyle (HsButton::noFocus);
  setBitmapId (0);
...
HsButton::~HsButton()
{
  ...
  if (HsButton::m_nRefs == 0) {
      ...
      if (m_hMsimg32 != NULL) {
          ...
          // Bugfix by C Young
          // http://www.codeproject.com/buttonctrl/HsButton.asp?msg=1610382#xx1610382xx
          m_hMsimg32 = NULL;
...
BEGIN_MESSAGE_MAP(HsButton, CButton)
	//{{AFX_MSG_MAP(HsButton)
  ON_WM_ERASEBKGND()
  ON_WM_KILLFOCUS()
  ON_WM_LBUTTONDOWN()
  ON_WM_LBUTTONUP()
  ...
  (HsButton::Type type)
{
  ...
  // Set button type
  switch (type) {
    case HsButton::staticButton:
      m_bStatic = true;
      break;

    case HsButton::checkBox:
    case HsButton::radio:
    case HsButton::checkButton:
    case HsButton::pushButton:
      break;

    case HsButton::pushButtonDropDown:
      m_bDropDown = true;
      break;

    case HsButton::hotCheckButton:
    case HsButton::hotPushButton:
      m_bHot = true;
      break;

    case HsButton::hotPushButtonDropDown:
      m_bHot = true;
      m_bDropDown = true;
      break;

    case HsButton::pushButtonMulti:
      m_bMulti = true;
      break;

    case HsButton::hotPushButtonMulti:
      m_bHot = true;
      m_bMulti = true;
      break;

    case HsButton::hyperlink:
      m_bHyperlink = true;
      break;
  ...
  // Reset checked mode
  if ((m_type != HsButton::checkButton) &&
      (m_type != HsButton::hotCheckButton))
     m_bChecked = false;
...
void HsButton::setTextStyle
  (HsButton::Text textStyle)
{
  // Reset text styles
  m_bText = false;
  m_bCenter = false;
  ...
  switch (textStyle) {
    ...
    case HsButton::singleLine:
      m_bText = true;
      break;

    case HsButton::singleLineCenter:
      m_bText = true;
      m_bCenter = true;
      break;

    case HsButton::multiLine:
      m_bText = true;
      m_bMultiLine = true;
      break;
...
void HsButton::setFocusStyle
  (HsButton::Focus focusStyle)
{
  ...
  (bool bChecked)
{
  if ((m_type == HsButton::checkButton) ||
      (m_type == HsButton::hotCheckButton) ||
      (m_type == HsButton::checkBox) ||
      (m_type == HsButton::radio)) {
      m_bChecked = bChecked;
      HWND hWnd = GetSafeHwnd();
      if ((hWnd != NULL) && ::IsWindow (hWnd))
         Invalidate();
  ...
  if (isChecked() && !m_strButtonGroup.IsEmpty()) {
      ...
      for (int nIndex=0; (nIndex < nButtons); nIndex++) {
          ...
          if (hWnd != GetSafeHwnd()) {
              if (IsWindow (hWnd)) {
                  HsButton* pButton = (HsButton *) CWnd::FromHandle (hWnd);
                  ASSERT (pButton != NULL);
...
void HsButton::clearMultiClick()
{
  if ((m_type == HsButton::pushButtonMulti) ||
      (m_type == HsButton::hotPushButtonMulti)) {
      m_bMultiClicked = false;
      HWND hWnd = GetSafeHwnd();
      if ((hWnd != NULL) && ::IsWindow (hWnd))
         Invalidate();
...
//! @param    pGroup    The HsButtonGroup.
//! @return   true if successful, false otherwise.
bool HsButton::removeFromGroup()
{
...
/////////////////////////////////////////////////////////////////////////////
// HsButton message handlers

void HsButton::PreSubclassWindow() 
{
...
void HsButton::OnKillFocus(CWnd* pNewWnd)
{
  HWND hWnd = GetSafeHwnd();
  if ((hWnd != NULL) && ::IsWindow (hWnd))
     Invalidate();
...
void HsButton::OnLButtonDown (UINT nFlags, CPoint point)
{
 // multiHitTest (point);   // sets m_bMultiClicked
  if(m_pEventHandler != NULL)
  {
	  if(m_pEventHandler->OnLHsButtonDown(this, nFlags, point) != 0)
	  {
...
void HsButton::OnLButtonUp (UINT nFlags, CPoint point)
{
  ...
  /*if ((m_type == HsButton::checkButton) ||
      ...
      if (rectButton.PtInRect (point))
         if ((m_type != HsButton::radio) && m_strButtonGroup.IsEmpty())
            check (!isChecked());
         else
            check (true);
  }*/
  ...
  if(m_pEventHandler != NULL)
  {
	  if(m_pEventHandler->OnLHsButtonUp(this, nFlags, point) != 0)
	  {
...
void HsButton::OnMouseMove(UINT nFlags, CPoint point) 
{
...
BOOL HsButton::OnEraseBkgnd(CDC* pDC) 
{
...
BOOL HsButton::OnSetCursor (CWnd* /* pWnd */, UINT /* nHitTest */, UINT /* message */)
{
  if (m_bHyperlink) {
...
void HsButton::OnSetFocus(CWnd* pOldWnd)
{
  HWND hWnd = GetSafeHwnd();
  if ((hWnd != NULL) && ::IsWindow (hWnd))
     Invalidate();
...
void HsButton::DrawItem (LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
  CDC* pDC = CDC::FromHandle (lpDrawItemStruct->hDC);
  ASSERT (pDC != NULL);

  // Draw frame for non-radio and non-checkbox buttons
  if ((m_type != HsButton::checkBox) && (m_type != HsButton::radio))
...
/////////////////////////////////////////////////////////////////////////////
// HsButton helper functions

//! Draws the button's frame
void HsButton::drawFrame
  (CDC* pDC,
...
void HsButton::drawRadioCheckFrame
  (CDC* pDC,
   LPDRAWITEMSTRUCT lpDrawItemStruct,
   int& nLeftEdge)
{
  ...
  // Draw frame
  UINT uFrameCtrl = (m_type == HsButton::checkBox ? DFCS_BUTTONCHECK : DFCS_BUTTONRADIO);
  if (m_bChecked)
     uFrameCtrl |= DFCS_CHECKED;
   ...
   int& nBitmapRightEdge)
{
  ...
  GetClientRect (&rectButton);
  CPoint pt;
  if (m_bText || (m_type == HsButton::checkBox) || (m_type == HsButton::radio))
...
void HsButton::drawMultiDropDownRegion
  (CDC* pDC,
   LPDRAWITEMSTRUCT lpDrawItemStruct)
{
  ASSERT (pDC != NULL);
   ...
   int nRightEdge)
{
  ...
  if (!m_bStatic && !m_bHot && (getFocusStyle() != HsButton::noFocus))
...
void HsButton::drawDefaultBorder
  (CDC* pDC,
   LPDRAWITEMSTRUCT lpDrawItemStruct)
{
  ASSERT (pDC != NULL);
  ...
  HsButton::Type btnType = getType();
  if ((btnType != HsButton::pushButton) &&
      (btnType != HsButton::pushButtonDropDown) &&
      (btnType != HsButton::pushButtonMulti))
     return;

  // Nothing to do if the default border isn't required
  if (getFocusStyle() != HsButton::defaultFocus)
     return;
  ...
  if (hDC) {
       ...
       if (bwDC) {
            ...
            if (hBitmapBW) {
                 ...
                 // http://www.codeproject.com/buttonctrl/HsButton.asp?msg=1547083#xx1547083xx
...
void HsButton::drawDropDownIndicator
  (HDC      hdcDest,            // destination DC
...
void HsButton::drawHotButtonFrame
  (LPDRAWITEMSTRUCT lpDrawItemStruct)
{
  // Get DC
  ASSERT (lpDrawItemStruct != NULL);
...
void HsButton::drawGradientRect
  (CDC *pDC, CRect r,
   COLORREF cLeft,
