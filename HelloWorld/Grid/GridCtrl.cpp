...
void CGridCtrl::OnSettingChange(UINT uFlags, LPCSTR lpszSection)
{
...
void CGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol, LPCSTR strText)
{
...
int CGridCtrl::InsertColumn(LPCSTR strHeading,
...
int CGridCtrl::InsertRow(LPCSTR strHeading, int nRow /* = -1 */)
{
...
BOOL CGridCtrl::SetItemText(int nRow, int nCol, LPCSTR str)
{
    if (GetVirtualMode())
        return FALSE;
...
BOOL CGridCtrl::SetItemTextFmt(int nRow, int nCol, LPCSTR szFmt, ...)
{
    if (GetVirtualMode())
        return FALSE;
        ...
        (m_bLMouseButtonDown && m_MouseMode == MOUSE_NOTHING))
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING)
        {
            ...
            if (m_bTitleTips)
            {
                ...
                if (pCell)
                {
                    LPCSTR szTipText = pCell->GetTipText();
...
BOOL CGridCtrl::Save(LPCSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
    CStdioFile File;
    CFileException ex;
...
BOOL CGridCtrl::Load(LPCSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
    if (GetVirtualMode())
        return FALSE;
...
CSize CGridCtrl::GetTextExtent(int nRow, int nCol, LPCSTR str)
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    if (!pCell)
        return CSize(0, 0);
...
// virtual
BOOL CGridCtrl::ValidateEdit(int nRow, int nCol, LPCSTR str)
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
